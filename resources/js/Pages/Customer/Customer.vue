...
function showCustomerDetails(customerId) {
  axios.get(`/customer/${customerId}/show`)
    .then(response => {
...
const bulkDelete = () => {
  ...
  Swal.fire({
    title: 'Are you sure?',
    text: `Do you want to delete ${selectedIds.value.length} selected customers?`,
    icon: 'warning',
      ...
      .then((response) => {
        ...
        if (totalFailed > 0) {
          ...
          Object.entries(failed).forEach(([id, err]) => {
            failMessages += `Customer #${id}: ${err}<br>`;
          });
          Swal.fire({
            icon: 'warning',
            title: 'Bulk Delete Complete with Errors',
            html: `Deleted ${succeeded.length} customers.<br>${totalFailed} failed:<br><div style="text-align: left; margin-top: 10px; padding: 10px; background: #fee2e2; border-radius: 6px; font-size: 12px; color: #b91c1c; max-height: 150px; overflow-y: auto;">${failMessages}</div>`,
            confirmButtonColor: '#2e2c92'
          }).then(() => {
            location.reload();
          });
        } else {
          Swal.fire('Deleted!', 'Selected customers deleted successfully.', 'success').then(() => {
            location.reload();
          });
        }
      })
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      axios.delete(`/customer/destroy/${customerId}`)
        ...
        .catch((error) => {
          const errMsg = error.response?.data?.message || 'Failed to delete the customer. Please try again.';
...
<template>
  ...
  <div v-if="selectedIds.length > 0" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-white border border-gray-200 rounded-2xl shadow-xl px-6 py-4 flex items-center gap-6">
    <span class="text-sm font-semibold text-gray-700">{{ selectedIds.length }} customers selected</span>
    ...
    <div v-if="isViewModalOpen"
      ...
      <div class="bg-white p-8 rounded-2xl shadow-2xl max-w-2xl w-full my-auto transform transition-all duration-300 border border-gray-100 space-y-6">
        <div class="flex justify-between items-center pb-3 border-b border-gray-100">
          <h2 class="text-2xl font-bold text-[#292688]">Customer Details</h2>
        ...
        <div class="space-y-6 text-black text-left overflow-y-auto max-h-[70vh]">
          ...
          <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
              <div>
                <span class="block text-gray-500 text-xs font-semibold uppercase tracking-wider">GST Number</span>
                <span class="text-base font-medium">{{ viewData.gst_number || '---' }}</span>
              </div>
              <div>
