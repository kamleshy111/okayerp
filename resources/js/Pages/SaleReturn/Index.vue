...
const columns = [
  ...
  { data: 'return_no', title: 'Return No', className: 'whitespace-nowrap' },
  {
    ...
    title: 'Sale Invoice',
    className: 'whitespace-nowrap',
    render: (data) => `<span class="whitespace-nowrap">Invoice #${data}</span>`
  },
  { data: 'customerName', title: 'Customer', className: 'whitespace-nowrap' },
  {
    data: 'refund_amount',
    title: 'Refund Amount',
    className: 'whitespace-nowrap',
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      axios.delete(`/sale-return/destroy/${returnId}`)
        .then(() => {
          Swal.fire('Deleted!', 'Your sale return has been deleted.', 'success');
          location.reload();
        })
        .catch((error) => {
          const errMsg = error.response?.data?.message || 'Failed to delete sale return.';
  ...
  <AuthenticatedLayout>
    <div class="p-4 sm:p-6 space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl sm:text-3xl font-bold text-slate-900">Sales Returns</h1>
          <p class="text-xs sm:text-sm text-slate-500 mt-1">Manage product returns and refund transactions</p>
        </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6">
        <div class="overflow-x-auto">
          <!-- DataTable Component -->
          <DataTable :data="returns" :columns="columns" :options="dtOptions" id="sale_return" class="w-full min-w-[850px]">
