...
const bulkDelete = () => {
  ...
  Swal.fire({
    title: 'Are you sure?',
    text: `Do you want to delete ${selectedIds.value.length} selected expenses?`,
    icon: 'warning',
      ...
      .then((response) => {
        ...
        if (totalFailed > 0) {
          ...
          Object.entries(failed).forEach(([id, err]) => {
            failMessages += `Expense #${id}: ${err}<br>`;
          });
          Swal.fire({
            icon: 'warning',
            title: 'Bulk Delete Complete with Errors',
            html: `Deleted ${succeeded.length} expenses.<br>${totalFailed} failed:<br><div style="text-align: left; margin-top: 10px; padding: 10px; background: #fee2e2; border-radius: 6px; font-size: 12px; color: #b91c1c; max-height: 150px; overflow-y: auto;">${failMessages}</div>`,
            confirmButtonColor: '#2e2c92'
          }).then(() => {
            location.reload();
          });
        } else {
          Swal.fire('Deleted!', 'Selected expenses deleted successfully.', 'success').then(() => {
            location.reload();
          });
        }
      })
  ...
  <AuthenticatedLayout>
    <div class="p-6">
      ...
      <div class="overflow-x-auto mt-10">
        <!-- DataTable Component -->
        <DataTable :data="expenses" :columns="columns" :options="dtOptions" id="expense">
    ...
    <div v-if="selectedIds.length > 0" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-white border border-gray-200 rounded-2xl shadow-xl px-6 py-4 flex items-center gap-6">
      <span class="text-sm font-semibold text-gray-700">{{ selectedIds.length }} expenses selected</span>
