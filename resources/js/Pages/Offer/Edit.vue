...
const form = ref({
  ...
  target_amount: props.offer.target_amount,
  start_date: props.offer.start_date,
  end_date: props.offer.end_date,
  reward_description: props.offer.reward_description || '',
});
...
const submitForm = () => {
  ...
  router.post(route('offer.update', props.offer.id), form.value, {
    onSuccess: () => {
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="bg-white p-8 rounded-xl shadow-md max-w-2xl mx-auto space-y-6">
      ...
      <div>
        <h2 class="text-2xl font-bold text-[#292688]">Edit Target Offer</h2>
        <p class="text-sm text-gray-500 mt-1">Update target offer metrics</p>
      </div>
