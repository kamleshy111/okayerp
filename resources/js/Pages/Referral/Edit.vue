...
const form = ref({
  name: props.referralUser.name,
  phone: props.referralUser.phone || '',
  email: props.referralUser.email || '',
  notes: props.referralUser.notes || '',
});
...
const submitForm = () => {
  ...
  router.post(route('referral-user.update', props.referralUser.id), form.value, {
    onSuccess: () => {
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="bg-white p-8 rounded-xl shadow-md max-w-2xl mx-auto space-y-6">
      ...
      <div>
        <h2 class="text-2xl font-bold text-[#292688]">Edit Referral User</h2>
        <p class="text-sm text-gray-500 mt-1">Update referral user entry details</p>
      </div>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="flex justify-end gap-3 pt-2">
          ...
          >
            {{ isSubmitting ? 'Saving...' : 'Update Referral User' }}
          </button>
        </div>
      </form>
    </div>
