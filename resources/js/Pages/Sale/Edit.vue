...
const resetToCurrentDateTime = () => {
  ...
  const d = new Date();
  form.value.sale_time = `${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
};
...
const checkInvoiceNumber = (val) => {
  ...
  const v = (val || '').trim();
  if (!v || v === (sales.invoice_no || '').trim()) return invoiceError.value = '';
  invoiceTimer = setTimeout(async () => {
    try {
      const res = await axios.get(`/sale/check-invoice-no?invoice_no=${encodeURIComponent(v)}&exclude_id=${form.value.id}`);
      invoiceError.value = res.data?.available ? '' : (res.data?.message || 'Invoice number already exists in this store.');
    } catch (e) {}
  }, 250);
};
...
const openPaymentModal = () => {
  if (invoiceError.value) return toast.error(invoiceError.value);
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm space-y-6">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="relative">
            <button type="button" @click.stop="showDateTimePicker = !showDateTimePicker"
              class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-semibold bg-slate-100 hover:bg-amber-50 hover:text-amber-800 text-slate-700 border border-slate-200 hover:border-amber-200 transition cursor-pointer shadow-sm"
            ...
            <div v-if="showDateTimePicker" @click.stop
              ...
              <div>
                ...
                <input type="time" v-model="form.sale_time"
                  class="w-full border border-slate-200 px-3 py-1.5 rounded-lg text-xs focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white text-black" />
              </div>
              <div class="flex items-center justify-between pt-1">
                <button type="button" @click="resetToCurrentDateTime"
      ...
      <div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label class="block text-slate-700 text-sm font-semibold mb-2">Customer <span
                class="text-red-500">*</span></label>
            <vSelect v-model="form.customer_id" :options="customers" label="name" :reduce="customer => customer.id"
              placeholder="Search or select customer" class="w-full text-black bg-white" @search="onCustomerSearch"
          ...
          <div>
            <label class="block text-slate-700 text-sm font-semibold mb-2">Invoice No.</label>
            <input type="text" v-model="form.invoice_no" @input="checkInvoiceNumber($event.target.value)"
              placeholder="e.g. 1/2026-27" :class="[
                'w-full border px-3 py-2 rounded-xl focus:outline-none text-black text-sm shadow-sm font-medium',
                invoiceError ? 'border-red-500 bg-red-50' : 'border-slate-200 bg-white'
              ]" />
            <p v-if="invoiceError" class="text-xs text-red-600 mt-1">{{ invoiceError }}</p>
          </div>
          <div>
            <label class="block text-slate-700 text-sm font-semibold mb-2">Referral User (Optional)</label>
            <vSelect v-model="form.referral_user_id" :options="referralUsers" label="name" :reduce="user => user.id"
              placeholder="Search or select referral user" class="w-full text-black bg-white"
        ...
        <div v-if="isInternationalCustomer"
          class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 p-4 bg-purple-50/30 border border-purple-100 rounded-xl">
          <div>
          ...
          <div>
            <label class="block text-slate-700 text-sm font-semibold mb-2">Exchange Rate (1 {{ form.currency }} = ?
              INR)</label>
            <input type="number" v-model="form.exchange_rate" step="any" min="0.0001"
              class="w-full border border-slate-200 px-3 py-2 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition bg-white text-black text-sm shadow-sm" />
          </div>
        </div>

        <div v-if="selectedCustomer"
          class="mt-6 p-5 rounded-2xl bg-indigo-50/30 border border-indigo-100/50 shadow-sm transition-all duration-300">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm text-slate-700">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <span class="font-medium text-slate-800 truncate max-w-[200px]" :title="selectedCustomer.email">{{
                  selectedCustomer.email || 'Not Provided' }}</span>
              </div>
            </div>
            <div class="flex items-center gap-3">
              <div class="p-2.5 rounded-xl bg-white border border-indigo-100/80 text-indigo-600 shadow-sm">
              ...
              <div>
                ...
                <span class="font-medium text-slate-800 truncate max-w-[250px]" :title="selectedCustomer.address">{{
                  selectedCustomer.address || 'Not Provided' }}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <tr v-for="(item, index) in form.sale_items" :key="index" class="hover:bg-slate-50/40 transition">
                <td class="border-t border-slate-100 px-4 py-4 min-w-[220px]">
                  <div class="relative w-full">
                    <input type="text" v-model="item.temp_product_name" @focus="onProductInputFocus(index)"
                      @input="onProductInputChanged(index, $event.target.value)"
                      @keydown.down.prevent="navigateSidebar(1)" @keydown.up.prevent="navigateSidebar(-1)"
                      @keydown.enter.prevent="selectHighlightedProduct" @keydown.tab="closeSidebar"
                      @keydown.esc="closeSidebar" placeholder="Search product..."
                      class="w-full border border-slate-200 px-3 py-2 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition bg-white text-black text-sm shadow-sm" />
                  </div>
                  <div v-if="page.props.auth?.user?.allow_provide_additional_descriptions && item.description"
                    class="mt-1.5">
                    <div
                      class="text-xxs text-slate-500 bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-100 flex items-center justify-between shadow-xxs">
                      <span class="truncate max-w-[200px]" :title="item.description">{{ item.description }}</span>
                      <button type="button" @click="openDescriptionPopup(index)"
                        class="text-indigo-600 hover:text-indigo-800 ml-2 shrink-0">
                ...
                <td v-if="useAlternateUnits" class="border-t border-slate-100 px-4 py-4 min-w-[150px]">
                  <div v-if="item.alternate_unit_type" class="flex flex-col gap-2">
                    <div class="flex items-center gap-1.5">
                      <input :id="'alt-qty-input-' + index" type="number" step="any" v-model="item.alternate_quantity"
                    ...
                    <div v-if="activeEditingAltRow === index" @focusout="handleAltFocusOut($event, index)"
                      class="flex items-center gap-1 text-xs text-slate-400">
                      <input :id="`width-input-${index}`" type="number" step="any" v-model="item.width"
                      ...
                      <input type="number" step="any" v-model="item.height"
                        class="w-10 px-1 py-1 border border-slate-200 rounded-lg text-center text-black bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none shadow-sm transition placeholder:text-slate-300"
                        placeholder="H" />
                    </div>
                    <div v-else @click="startEditingAlt(index)"
                ...
                <td class="border-t border-slate-100 px-4 py-4">
                  <span
                    class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200/50 uppercase">{{
                      item.unit_type || 'pcs' }}</span>
                </td>
                <td class="border-t border-slate-100 px-4 py-4">
        ...
        <div class="md:hidden space-y-4">
          <div v-for="(item, index) in form.sale_items" :key="'mobile-' + index"
            ...
            <div class="space-y-3">
              <div>
                ...
                <div class="relative w-full">
                  <input type="text" v-model="item.temp_product_name" @focus="onProductInputFocus(index)"
                    ...
                    @keydown.down.prevent="navigateSidebar(1)" @keydown.up.prevent="navigateSidebar(-1)"
                    @keydown.enter.prevent="selectHighlightedProduct" @keydown.tab="closeSidebar"
                    @keydown.esc="closeSidebar" placeholder="Search or select product"
                ...
                <div v-if="page.props.auth?.user?.allow_provide_additional_descriptions && item.description"
                  class="mt-1.5">
                  <div
                    class="text-xs text-gray-500 bg-gray-50 px-3 py-1.5 rounded-xl border border-gray-100 flex items-center justify-between">
                    <span class="truncate max-w-[250px]" :title="item.description">{{ item.description }}</span>
                    <button type="button" @click="openDescriptionPopup(index)"
                      class="text-indigo-600 hover:text-indigo-800 ml-1 shrink-0">
                      <i class="bi bi-pencil-square"></i>
                    </button>
                  </div>
                </div>
              ...
              <div v-if="useAlternateUnits && item.alternate_unit_type"
                class="grid grid-cols-3 gap-2 bg-gray-100 p-2 rounded-lg border border-gray-200">
                <div>
                  <label class="block text-xxs font-semibold text-gray-500 mb-1">Alt Qty ({{ item.alternate_unit_type
                  }})</label>
                  <input type="number" step="any" v-model="item.alternate_quantity"
                    class="w-full px-2 py-1 border border-gray-300 rounded text-sm bg-white text-black"
                    placeholder="Alt Qty" />
                </div>
                <div>
                  <label class="block text-xxs font-semibold text-gray-500 mb-1">Width</label>
                  <input type="number" step="any" v-model="item.width"
                    class="w-full px-2 py-1 border border-gray-300 rounded text-sm bg-white text-black text-center"
                    placeholder="W" />
                </div>
                <div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" v-model="item.quantity" required @keydown.enter.prevent="moveToNextInput"
                ...
                <div>
                  ...
                  <input type="number" v-model="item.price" required @keydown.enter.prevent="moveToNextInput"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#292688] focus:outline-none transition text-sm"
                    placeholder="Price" />
                </div>
              </div>
              ...
              <div
                ...
                <div>
                  ...
                  <span class="text-gray-800 font-bold">{{ currencySymbol }} {{ ((parseFloat(item.quantity) || 0) *
        ...
        <div class="flex justify-end mt-8 border-t border-slate-100 pt-6">
          <button @click="openPaymentModal"
            class="w-full md:w-auto bg-gradient-to-r from-[#2E2C92] to-[#1c1a6e] hover:from-[#1c1a6e] hover:to-[#2E2C92] text-white px-8 py-3.5 rounded-2xl font-bold transition-all duration-300 shadow-md hover:shadow-lg flex items-center justify-center gap-2 hover:scale-[1.02] cursor-pointer">
    ...
    <div v-if="showPaymentModal"
      ...
      style="z-index: 9999;" @click.self="showPaymentModal = false">
      <form @submit.prevent="submitForm"
        class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md my-auto transform transition-all duration-300 border border-gray-100 space-y-4">
        <div class="flex justify-between items-center pb-2 border-b border-gray-100">
        ...
        <div class="space-y-4 border-t pt-4">
          ...
          <div v-if="props.allocatedPayment > 0" class="flex justify-between items-center text-sm text-gray-500">
            ...
            <span class="font-medium text-[#292688]">{{ currencySymbol }} {{ (parseFloat(props.allocatedPayment) /
              (parseFloat(form.exchange_rate) || 1.0)).toFixed(2) }}</span>
          </div>
          ...
          <div v-if="finalBalance?.type === 'advance'" class="flex justify-between items-center">
            ...
            <span class="text-green-600 font-bold">{{ currencySymbol }} {{ typeof finalBalance.amount === 'number' ?
              finalBalance.amount.toFixed(2) : finalBalance.amount }}</span>
          </div>
          ...
          <div v-if="finalBalance?.type === 'due'" class="flex justify-between items-center">
            ...
            <span class="text-red-600 font-bold">{{ currencySymbol }} {{ typeof finalBalance.amount === 'number' ?
              finalBalance.amount.toFixed(2) : finalBalance.amount }}</span>
          </div>
          ...
          <div class="flex justify-end gap-3 pt-2">
            <button type="button" @click="showPaymentModal = false"
              class="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl transition cursor-pointer">Cancel</button>
    ...
    <div v-if="activeSearchIndex !== null" @click="closeSidebar"
      class="fixed inset-0 z-40 bg-black/10 transition-opacity">
    </div>
    <div v-if="activeSearchIndex !== null"
      class="fixed right-0 top-0 bottom-0 w-96 bg-slate-50 border-l border-slate-200 shadow-2xl z-50 flex flex-col transition-all duration-300 transform translate-x-0">
      <!-- Sidebar Header -->
      ...
      <div class="flex-1 overflow-y-auto p-2 space-y-1">
        <div v-for="(product, idx) in products" :key="product.id" :id="'sidebar-item-' + idx"
          @click="selectProduct(product)" @mouseenter="selectedSidebarProductIndex = idx" :class="[
          ...
          <div class="flex flex-col text-left pr-4">
            ...
            <span class="text-xxs opacity-75 mt-0.5"
              :class="idx === selectedSidebarProductIndex ? 'text-slate-200' : 'text-slate-500'">
          ...
          <div class="text-right flex flex-col items-end shrink-0">
            ...
            <span class="text-xxs opacity-75"
              :class="idx === selectedSidebarProductIndex ? 'text-slate-200' : 'text-slate-500'">
              Stock Qty
            </span>
          </div>
    ...
    <div v-if="showDescriptionPopup"
      class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div
        class="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-slate-100 overflow-hidden transform transition-all">
        <!-- Modal Header -->
        <div
          class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-white">
        ...
        <div class="p-6 space-y-4">
          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Item
              Description</label>
            <input type="text" v-model="tempDescription"
              ...
              ref="descriptionTextarea" @keydown.enter.prevent="saveDescriptionPopup" />
          </div>
        </div>
        <!-- Modal Footer -->
        <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
          <button type="button" @click="closeDescriptionPopup"
          ...
          <button type="button" @click="saveDescriptionPopup"
