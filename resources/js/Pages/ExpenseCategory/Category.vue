...
const bulkDelete = () => {
  ...
  Swal.fire({
    title: 'Are you sure?',
    text: `Do you want to delete ${selectedIds.value.length} selected expense categories?`,
    icon: 'warning',
      ...
      .then((response) => {
        ...
        if (totalFailed > 0) {
          ...
          Swal.fire({
            ...
            html: `Deleted ${succeeded.length} expense categories.<br>${totalFailed} failed:<br><div style="text-align: left; margin-top: 10px; padding: 10px; background: #fee2e2; border-radius: 6px; font-size: 12px; color: #b91c1c; max-height: 150px; overflow-y: auto;">${failMessages}</div>`,
            confirmButtonColor: '#2e2c92'
          }).then(() => {
            location.reload();
          });
        } else {
          Swal.fire('Deleted!', 'Selected expense categories deleted successfully.', 'success').then(() => {
            location.reload();
          });
        }
      })
  ...
  <AuthenticatedLayout>
    <div class="p-6">
      ...
      <div class="overflow-x-auto mt-10">
        <!-- DataTable Component -->
        <DataTable :data="categories" :columns="columns" :options="dtOptions" id="expense-category">
