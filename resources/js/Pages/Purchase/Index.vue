...
const handleFileChange = async (event) => {
    ...
    Swal.fire({
        ...
        text: 'Optimizing the image for faster upload. Please wait.',
    ...
    try {
        ...
        Swal.fire({
            ...
            text: 'Oeading your purchase invoice. Please wait.',
            allowOutsideClick: false,
            didOpen: () => {
                Swal.showLoading();
            }
        ...
        const response = await axios.post('/purchase/scan', formData, {
            headers: {
        ...
        if (response.data.success) {
            ...
            availableSuppliers.value = response.data.suppliers || [];
...
const previewGrandTotal = computed(() => {
    ...
    const transport = parseFloat(scanResult.value.transport) || 0;
    return previewTotalAmount.value + previewTotalGST.value + transport;
});
...
const saveScannedPurchase = async () => {
    isSaving.value = true;
    Swal.fire({
        title: 'Saving Purchase...',
        text: 'Adding purchase and updating stock...',
        allowOutsideClick: false,
        didOpen: () => {
            Swal.showLoading();
        }
    ...
    try {
        const payload = {
            supplier_id: scanResult.value.supplier_id,
            supplier_name: scanResult.value.supplier_name,
            invoice_no: scanResult.value.invoice_no,
            purchase_date: scanResult.value.purchase_date,
            transport: scanResult.value.transport,
        ...
        const response = await axios.post('/purchase/store-scanned', payload);
        Swal.close();

        await Swal.fire({
            icon: 'success',
            title: 'Success',
            text: response.data.message || 'Purchase added successfully!'
        });
  ...
  <div v-if="showPreviewModal" class="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col p-6 space-y-4">
          ...
          <div class="flex items-center justify-between border-b pb-3">
              <h3 class="text-2xl font-bold text-[#2e2c92]">Verify Scanned Invoice</h3>
          ...
          <div class="flex-1 overflow-y-auto pr-1 space-y-4 text-black">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-xl">
                  <div>
                      ...
                      <select v-model="scanResult.supplier_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 mt-1 text-black bg-white focus:ring-2 focus:ring-[#2e2c92] focus:outline-none">
                          <option :value="null">➕ Create New: {{ scanResult.supplier_name }}</option>
                          <option v-for="sup in availableSuppliers" :key="sup.id" :value="sup.id">
                              {{ sup.name }}
                          </option>
                      </select>
                      <span v-if="!scanResult.supplier_id" class="text-xs text-blue-600 mt-1 block">New supplier will be added.</span>
                  </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-xl">
                  <div>
                      <label class="block text-sm font-semibold text-gray-700">Amount Paid (optional)</label>
              ...
              <div class="flex justify-end">
                  <div class="w-full md:w-80 bg-gray-50 rounded-xl p-4 space-y-2 text-sm border">
                      ...
                      <div class="flex justify-between items-center">
                          ...
                          <input type="number" v-model="scanResult.transport" class="w-24 text-right border border-gray-300 rounded px-2 py-0.5 text-black bg-white focus:outline-none focus:ring-1 focus:ring-[#2e2c92]" />
                      </div>
          ...
          <div class="flex justify-end gap-3 border-t pt-3">
              ...
              <button @click="saveScannedPurchase" :disabled="isSaving" class="px-5 py-2 bg-[#2e2c92] hover:bg-[#201d70] text-white rounded-lg transition font-medium flex items-center gap-2">
