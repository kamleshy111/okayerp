...
const onEnterKey = (event) => {
  // If no suppliers are found, open modal directly
  if (supplierSearchQuery.value && suppliers.value.length === 0) {
    event.preventDefault();
...
onMounted(() => {
  ...
  // Auto-focus supplier search input on page load
  nextTick(() => {
...
// Watch showSupplierModal to manage focus
...
const availableSupplierDistricts = computed(() => {
...
<template>
    ...
    <div class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-[#292688] tracking-tight">Create Purchase</h2>
        </div>

        <div>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    <label class="block text-slate-700 font-semibold text-sm mb-2">Supplier <span class="text-red-500">*</span></label>
                        <vSelect
                                    ...
                                    <div class="px-3 py-2 text-slate-400 text-sm">
                                        ...
                                        <button
                                            ...
                                            :class="supplierSearchQuery
                                                ? 'mt-2 inline-flex items-center text-indigo-600 text-sm font-semibold border border-indigo-200 rounded-lg px-3 py-1.5 hover:bg-indigo-50 transition'
                                                : 'mt-2 inline-flex items-center text-indigo-600 text-sm font-semibold'"
                                        >
                                            ➕ Add New Supplier
                                        </button>
                                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
                <div v-if="selectedSupplier" class="mt-4 p-5 rounded-2xl bg-indigo-50/40 border border-indigo-100/60 shadow-sm space-y-1.5 transition-all">
                    ...
                    <p class="text-sm text-slate-800"><strong>Email:</strong> {{ selectedSupplier.email }}</p>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md my-auto transform transition-all duration-300 border border-gray-100 space-y-4">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" ref="newSupplierNameInput" v-model="newSupplier.name" class="w-full border border-gray-300 px-3 py-2 rounded-xl focus:ring-2 focus:ring-[#292688] focus:outline-none" />
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select v-model="newSupplier.country"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            type="text"
                            v-model="newSupplier.state"
                            class="w-full border border-gray-300 px-3 py-2 rounded-xl focus:ring-2 focus:ring-[#292688] focus:outline-none"
                            placeholder="Enter State"
                        />
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-4" v-if="!newSupplier.country || newSupplier.country === 'India'">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">District</label>
                        <vSelect
                            :options="availableSupplierDistricts"
        ...
        <form @submit.prevent="submitForm" novalidate class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md my-auto transform transition-all duration-300 border border-gray-100 space-y-4">
            ...
            <div class="space-y-4 border-t pt-4">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <input type="text" ref="paymentDiscountInput" v-model="form.transport" placeholder="0"
