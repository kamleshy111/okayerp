...
@php
  ...
  $store = $return->purchase->supplier && $return->purchase->supplier->user ? $return->purchase->supplier->user : null;
...
<div class="invoice-container">
  ...
  <table class="border-bottom header-table">
    <tr>
      ...
      <td class="company-details">
        <div class="bold text-center" style="font-size: 10px; margin-bottom: 2px; letter-spacing: 1px;">DEBIT NOTE / PURCHASE RETURN</div>
    ...
    <tr>
      <td class="border-right">
        <table class="meta-sub-table">
          <tr>
            <td class="bold" style="width: 35%;">Debit Note No.</td>
            <td style="width: 5%;">:</td>
          ...
          <tr>
            <td class="bold">Original Bill ID #</td>
            <td>:</td>
  ...
  <table class="border-bottom party-table">
    <tr>
      <td class="border-right">
        <div class="bold border-bottom" style="margin-bottom: 4px; padding-bottom: 2px;">From (Supplier) :</div>
        <div class="bold">{{ $return->purchase->supplier->name ?? 'N/A' }}</div>
        <div>{{ $return->purchase->supplier->address ?? 'N/A' }}</div>
        @if($return->purchase->supplier && $return->purchase->supplier->phone)
          <div>Phone: {{ $return->purchase->supplier->phone }}</div>
        @endif
        @if($return->purchase->supplier && $return->purchase->supplier->gst_number)
          <div class="bold" style="margin-top: 4px;">GSTIN / UIN : {{ $return->purchase->supplier->gst_number }}</div>
        @endif
      </td>
      <td>
        <div class="bold border-bottom" style="margin-bottom: 4px; padding-bottom: 2px;">Bill To (Store) :</div>
  ...
  <table class="items-table">
    ...
    <tbody>
      ...
      <tr class="total-row">
        <td class="border-right">&nbsp;</td>
        <td class="bold border-right text-right" colspan="2">Subtotal Return</td>
      ...
      <td class="border-right" style="width: 50%;">
        ...
        <div style="font-size: 9px; color: #333;">
          ...
          <ol class="terms-list">
            ...
            <li>Debit notes reflect purchase return adjustments to supplier accounts.</li>
          </ol>
        </div>
      </td>
      <td style="width: 50%; vertical-align: top; padding: 8px;">
        <div class="text-center" style="font-size: 10px; margin-bottom: 40px;">Store Representative Signature</div>
